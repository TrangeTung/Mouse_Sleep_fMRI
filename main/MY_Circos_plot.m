function F = MY_Circos_plot(data, opt)

% Plots correlation matrix as a MY_Circos_plot
%
% usage: vargout = MY_Circos_plot(data,opt);
%
%   MY_Circos_plot(data) data is a square numeric matrix with values in [0,1].
%
%                 NOTE: only the off-diagonal lower triangular section of data is
%                       considered, i.e. tril(data,-1).
%                                               
%     opt.label     Plot a MY_Circos_plot with custom labels at each node.
...
%
%   H = MY_Circos_plot(...) Returns a structure with handles to the graphic objects
%
...
%   MY_Circos_plot(x, repmat(('a':'j')',1,2))
%   MY_Circos_plot(x, {'Hi','how','is','your','day?', 'Do','you','like','MY_Circos_plots?','NO!!'})
%
%   % Customize curve colors
%   MY_Circos_plot([],[],[1,0,1;1 1 0])
%
%   % Customize node color
%   MY_Circos_plot([],[],[],[0,1,0])
%
...
% $Id: MY_Circos_plot.m 1077 2016-11-15 20:07:42Z gaser $
...
def.ncolor(:,:,:)  = [hot(sz(1)); winter(sz(1))];    % node colors (doughnut)
def.maxlinewidth   = 10;             % maximal line width for connections
...
def.ccolor         = parula(N*2);
%                             hsv2rgb([[linspace(.8333, .95, N); ones(1, N); linspace(1,0,N)],...
%                               [linspace(.03, .1666, N); ones(1, N); linspace(0,1,N)]]');
                            
opt = MY_update_struct_par(opt,def);
...
step  = tau/sz(1);
theta = (-1/4*tau : step : 3/4*tau - step)+step/2;
...
for c = 1:N2
    ...
    if nnz(idx)
        ...
        ss = plot(Bx(:),By(:),'Color',opt.ccolor(pos,:),'LineWidth',linewidth(pos));
        ss.Color(4) = 0.1;
    end
end
...
s.s        = scatter(x(isrt),y(isrt),[], opt.ncolor,'fill',...
    'MarkerEdgeColor',ecolor,'LineWidth',1,'MarkerFaceAlpha',.5);
...
function opt1 = MY_update_struct_par(opt,def)

field = fieldnames(def);
opt1 = def;

for iter = 1:numel(field)
    
    % eval(['a=opt.',field{iter},';']);
    
    try
        eval(['b=opt.',field{iter},';']);
        eval(['opt1.',field{iter},'=b;'])
    catch

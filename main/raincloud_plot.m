...
function [h, u] = raincloud_plot(X, Center, varargin)
...
h{1} = patch('XData',[Xi,Xi(end:-1:1)],'YData',[f*0,f(end:-1:1)]); hold on
...
if box_on
    ...
    for lp=1:6
        ...
        if lp==3
            Q=get(h{lp},'Position');
            set(h{lp},'Position',Q+[0 Center 0 0]);
        elseif lp==1
            Qx=get(h{lp},'xdata');
            Qy=get(h{lp},'ydata');
            set(h{lp},'xdata',Qx+0);
            set(h{lp},'ydata',Qy/3+Center);
        else
            Qx=get(h{lp},'xdata');
            Qy=get(h{lp},'ydata');
            set(h{lp},'xdata',Qx+0);
            set(h{lp},'ydata',Qy+Center);
        end
        % ylim([0 .030])
    end

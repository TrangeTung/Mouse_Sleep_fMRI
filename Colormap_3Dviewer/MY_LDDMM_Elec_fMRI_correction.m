function MY_LDDMM_Elec_fMRI_correction(ref,source,others,prefix)

% ref, source, others
% must be absolute path

% ref = fullfile(WholePath,'mean_T2.nii');
% source = fullfile(WholePath,'mean_EPI',Mdir(ml).name,'mean_nrrsm2dseq.nii');
%others = fullfile(WholePath,'nrrsm2dseq.nii');

Ref_img=ref;
Source_img=source;

% hdr=spm_vol(ref);
% Ref_img=spm_read_vols(hdr);
% 
% hdr=spm_vol(source);
% Source_img=spm_read_vols(hdr);


% [pathstr,name,suffix] = fileparts(others);




%% lddmm
LDDMM_X = zeros(size(Ref_img));
LDDMM_Y = zeros(size(Ref_img));

fmoving = Source_img*0;
for sl = 1:size(Ref_img,3)
    
    M = Source_img(:,:,sl);
    if ~isempty(find(M(:)>100))
        moving = M;

        fixed = Ref_img(:,:,sl);
        
        Jmatch=imhist(moving);
        fixed_=histeq(fixed,Jmatch);
        fixed_ = 255-fixed_;
        fixed_(fixed==0)=0;
        %fixed_=histeq(fixed_,Jmatch);

        %set(gcf,'visible','off');
        [Mp,sxl,syl,~] = lddmm_2D(fixed_,moving);
        %set(gcf,'visible','on');
...
hdr.fname = fullfile(pathstr,['mean',prefix,name,suffix]);
spm_write_vol(hdr,fmoving);



Xhdr(1) = hdr(1); Xhdr(1).n = [1,1];  Xhdr(1).dt = [16,0];
Xhdr(2) = hdr(1); Xhdr(2).n = [2,1];  Xhdr(2).dt = [16,0];
Xhdr(1).fname = fullfile(pathstr,'LDDMM.nii');
Xhdr(2).fname = fullfile(pathstr,'LDDMM.nii');
Xhdr(1).descrip = 'Vol 1: LDDMM_X, Vol 2: LDDMM_Y';
Xhdr(2).descrip = 'Vol 1: LDDMM_X, Vol 2: LDDMM_Y';
LDDMM = cat(4,LDDMM_X,LDDMM_Y);
spm_write_vol_4D(Xhdr,LDDMM);
...
hdr=spm_vol(others);
%Other_img=spm_read_vols(hdr);

LDDMM = spm_read_vols_4D(spm_vol(fullfile(pathstr,'LDDMM.nii')));
LDDMM_X = squeeze(LDDMM(:,:,:,1));
LDDMM_Y = squeeze(LDDMM(:,:,:,2));


Fhdr = spm_vol(others);
for tl=1:numel(Fhdr)
    Fhdr(tl).fname = fullfile(pathstr,[prefix,name,suffix]);
end


for tl=1:numel(hdr)
    img = spm_read_vols_4D(hdr(tl));
    ...
    for sl = 1:size(Ref_img,3)
        ...
        I0 = interpn(x,y,I,x_prime,y_prime,'linear',0); % moving image intensities at updated points
...
function [Mp,sxl,syl,lim] = lddmm_2D(I,J)

%% Parameters
niter           = 50;
sigma_fluid     = 2.0; % regularize update      field
sigma_diffusion = 3.0; % regularize deformation field
sigma_i         = 2.0; % weight on similarity term
sigma_x         = 4.0; % weight on spatial uncertainties (maximal step)
nlevel          = 7;   % multiresolution
do_display      = 1;   % display iterations
%% Load fixed image
F = I;%double(I)/max(double(I(:)))*105;          % fixed   image
%% Load moving image
M = J;%double(J)/max(double(J(:)))*255;
% M(M>105)=105;
...
if nlevel == 1
    ...
    for k=nlevel:-1:2
        ...
        [Mp,sxl,syl,vxl,vyl,lim] = register(Fl,Ml,opt);
        % upsample
        vx = imresize(vxl/(2^-(k-1)),size(M));
        vy = imresize(vyl/(2^-(k-1)),size(M));
    end
...
%% Register two images
function [Mp,sx,sy,vx,vy,lim] = register(F,M,opt)

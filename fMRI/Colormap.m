...
%     Func_Img_3D_raw = flip(permute(Func_Img_3D_raw,[1 3 2]),1);
else
...
%         Template_Img_3D = flip(permute(Template_Img_3D,[1 3 2]),1);
    ...
    %Img_RGB = MY_reshape_RGB_Img_multi_row(Img_RGB,slice,ceil(numel(slice)));%,6);
    ...
    Img_RGB = MY_reshape_RGB_Img_multi_row(Img_RGB_nothre,slice,ceil(numel(slice)));
...
if ~isempty(f_mask)
    ...
    mask_reshape = reshape(permute(flip(flip(img_reslice,2),1),[2 1 3]),[size(mask_reslice,2) size(mask_reslice,1)*numel(slice)]);
